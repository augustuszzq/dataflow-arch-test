...
# Log File Path
LOG_FILE=~/mixed_precision/scripts/new_llm/tarin_moe_withmixed.log
...
export MODEL_DIR=model_dir_mixtral
if [ -d "$MODEL_DIR" ]; then rm -Rf $MODEL_DIR; fi

# Step 7: Run the training script with the YAML file from the modelzoo directory
cd ~/mixed_precision/R_2.3.0/modelzoo/src/cerebras/modelzoo/models/nlp/mixtral

python run.py CSX \
  --job_labels name=mixtral \
  --params /home/kevienzzq/mixed_precision/R_2.3.0/modelzoo/src/cerebras/modelzoo/models/nlp/mixtral/configs/params_mixtral_8x7b.yaml \
  --num_csx=1 \
  --mode train \
  --model_dir $MODEL_DIR \
